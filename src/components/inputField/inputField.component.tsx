...
>((props, ref) => {
    ...
    }, [border, inputLength, lengths?.maxLength, lengths?.minLength, translateX, translateY, value]);
    ...
    return (
        <InputContainer>
            ...
            <BoxAnimated
                style={{
                    border: `${
                        borderless ? 'none' : `${(disabled && `1px solid #c1c1c1`) || (required && `${border}`)}`
                    }`,
                ...
                color={color}
            >
                {leftAdorn && <LeftAdornment style={leftStyle}>{leftAdorn}</LeftAdornment>}
                {required && (
                    <RequiredSignAnimated
                        style={{
                            translateY: translateY.value,
                            color: interpolate(translateY.value, [-4, 1], ['#ff0000', '#a1a1a1']),
                        }}
                    >
                        <FaStarOfLife size={7} />
                    </RequiredSignAnimated>
                ...
                <input
                    ...
                    {...rest}
                />
                {rightAdorn && <RightAdornment style={rightStyle}>{rightAdorn}</RightAdornment>}
                {type === 'password' && (
                    <EndAdornment
                        onClick={(e: any) => {
                            isPassword(e);
                ...
                {clear ? (
                    <EndAdornment>
                        {loading ? <Loader style={loaderStyle} /> : <TiBackspace onClick={clear} />}
                    </EndAdornment>
                ) : (
                    loading && (
                        <EndAdornment>
                ...
                {inputCount && (
                    <InputLengthAnimated
                        style={{
                            ...
                            color: interpolate(translateX.value, [0, 4], ['#717171', '#ff0000']),
                        }}
                    >
                        {inputLength}
                        {max && <> / {max}</>}
            ...
            >
                {required && (
                    <RequiredSignAnimated
                        style={{
                            translateY: translateY.value,
                            color: interpolate(translateY.value, [-4, 0], ['#ff0000', '#818181']),
                        }}
                    >
                        <FaStarOfLife size={7} />
                    </RequiredSignAnimated>
                ...
                />
                {clear ? (
                    <EndAdornment style={{ position: 'absolute', top: '5px', right: '5px' }}>
                        {loading ? <Loader style={loaderStyle} /> : <TiBackspace onClick={clear} />}
                    </EndAdornment>
                ) : (
                    loading && (
                ...
                {inputCount && (
                    <InputLengthAnimated
                        style={{
                            fontSize: '10px',
                            translateX: translateX.value,
                            color: interpolate(translateX.value, [0, 4], ['#818181', '#ff0000']),
                        }}
                    >
                        {inputLength}
                        {maxLength && <> / {maxLength}</>}
...
>((props, ref) => {
    ...
    return (
        <InputContainer>
            ...
            <BoxAnimated
                style={{
                    border: `${borderless ? 'none' : `${disabled && `1px solid #c1c1c1`}`}`,
                    color: `${(loading || disabled) && `1px solid #c1c1c1`}`,
                    ...style,
                }}
                className={className}
            ...
            >
                {!right && (
                    <LeftAdornment style={{ ...leftStyle, width: 40, aspectRatio: '1/1' }}>
                ...
                />
                {right && (
                    <RightAdornment style={{ ...rightStyle, width: 40, aspectRatio: '1/1' }}>
                        {loading ? <Loader style={loaderStyle} /> : <AiOutlineSearch />}
                    </RightAdornment>
                )}
